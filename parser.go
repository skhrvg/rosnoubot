...
func calculateWeekdays(xlFile *xlsx.File) ([6]int, [6]bool) {
	...
	for index, row := range xlFile.Sheets[0].Rows {
		...
		case "ПОНЕДЕЛЬНИК":
			weekdayStart[0] = index + 1
		case "ВТОРНИК":
			weekdayStart[1] = index + 1
		case "СРЕДА":
			weekdayStart[2] = index + 1
		case "ЧЕТВЕРГ":
			weekdayStart[3] = index + 1
		case "ПЯТНИЦА":
			weekdayStart[4] = index + 1
		case "СУББОТА":
			weekdayStart[5] = index + 1
		}
	}
	for i := 0; i < 5; i++ {
		if weekdayStart[i+1]-weekdayStart[i] < 4 {
			weekdayDayoff[i] = true
		}
	}
		...
		for i := row; i < lastRow; i += 3 { //Предмет
			...
			for j := startCol; j <= lastCol; j++ { // Дни
				if xlFile.Sheets[0].Rows[i].Cells[j].Value != "" {
					...
					case "Л":
						newEntry.workType = "Лекция"
					case "С":
						newEntry.workType = "Семинар"
					case "ПЗ":
						newEntry.workType = "Практическое занятие"
					case "ЗАЧ":
						newEntry.workType = "ЗАЧЕТ"
					case "Л/ПЗ":
						newEntry.workType = "Лекция / Практическое занятие"
					case "Л/С":
						newEntry.workType = "Лекция / Семинар"
					case "Лаб":
						newEntry.workType = "ЛАБОРАТОРНАЯ РАБОТА"
					case "ДИФ.ЗАЧ":
						newEntry.workType = "ДИФ. ЗАЧЕТ"
					case "ЗАЩ":
						newEntry.workType = "ЗАЩИТА"
					case "С/Л":
						newEntry.workType = "Семинар / Лекция"
					case "ПЗ/Л":
						newEntry.workType = "Практическое занятие / Лекция"
					case "Л/ЗАЧ":
						newEntry.workType = "Лекция / ЗАЧЕТ"
					case "К":
						newEntry.workType = "КОНСУЛЬТАЦИЯ"
					case "ЭКЗ":
